...
export default function LinkCard({links, tags, fetchLinks, changeTagFunc}:{links:any, tags:any, fetchLinks:Function, changeTagFunc:Function}) {
            ...
            <Modal isOpen={isChangeTagModalOpen} setIsOpen={setIsChangeTagModalOpen} title="Change Tag">
                ...
                <div className="flex flex-col gap-2">
                    {tags && tags.map((tag:any) => {
                        return(
                            <button key={tag.id} onClick={()=>changeTag(id, tag.id)} className="btn-secondary">{tag.name}</button>
                        )
                    })}
                </div>
            </Modal>
